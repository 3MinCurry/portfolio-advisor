"""
Retirement Specialist Agent - provides retirement planning analysis and projections.
"""

import os
import random
import logging
from typing import Dict, Any, Optional

from agents.extensions.models.litellm_model import LitellmModel
from src.portfolio import calculate_asset_allocation, calculate_portfolio_value

logger = logging.getLogger()

# Annualized return assumptions
EQUITY_RETURN_MEAN, EQUITY_RETURN_STD = 0.07, 0.18
BOND_RETURN_MEAN, BOND_RETURN_STD = 0.04, 0.05
REAL_ESTATE_RETURN_MEAN, REAL_ESTATE_RETURN_STD = 0.06, 0.12
COMMODITIES_RETURN_MEAN, COMMODITIES_RETURN_STD = 0.05, 0.15
ALTERNATIVES_RETURN_MEAN, ALTERNATIVES_RETURN_STD = 0.07, 0.16
CASH_RETURN = 0.02
INFLATION_RATE = 0.03
RETIREMENT_HORIZON_YEARS = 30


def _blended_expected_return(allocation: Dict[str, float]) -> float:
    return (
        allocation.get("equity", 0) * EQUITY_RETURN_MEAN
        + allocation.get("bonds", 0) * BOND_RETURN_MEAN
        + allocation.get("real_estate", 0) * REAL_ESTATE_RETURN_MEAN
        + allocation.get("commodities", 0) * COMMODITIES_RETURN_MEAN
        + allocation.get("alternatives", 0) * ALTERNATIVES_RETURN_MEAN
        + allocation.get("cash", 0) * CASH_RETURN
    )


def _random_portfolio_return(allocation: Dict[str, float], rng: random.Random) -> float:
    return (
        allocation.get("equity", 0) * rng.gauss(EQUITY_RETURN_MEAN, EQUITY_RETURN_STD)
        + allocation.get("bonds", 0) * rng.gauss(BOND_RETURN_MEAN, BOND_RETURN_STD)
        + allocation.get("real_estate", 0) * rng.gauss(REAL_ESTATE_RETURN_MEAN, REAL_ESTATE_RETURN_STD)
        + allocation.get("commodities", 0) * rng.gauss(COMMODITIES_RETURN_MEAN, COMMODITIES_RETURN_STD)
        + allocation.get("alternatives", 0) * rng.gauss(ALTERNATIVES_RETURN_MEAN, ALTERNATIVES_RETURN_STD)
        + allocation.get("cash", 0) * CASH_RETURN
    )


def run_monte_carlo_simulation(
    current_value: float,
    years_until_retirement: int,
    target_annual_income: float,
    asset_allocation: Dict[str, float],
    annual_contribution: float = 10_000.0,
    num_simulations: int = 500,
    seed: Optional[int] = None,
) -> Dict[str, Any]:
    """Run Monte Carlo simulation for retirement planning."""
    successful_scenarios = 0
    final_values = []
    years_lasted = []

    for sim in range(num_simulations):
        rng = random.Random(seed + sim) if seed is not None else random.Random()
        portfolio_value = current_value

        for _ in range(years_until_retirement):
            portfolio_value = portfolio_value * (1 + _random_portfolio_return(asset_allocation, rng))
            portfolio_value += annual_contribution

        annual_withdrawal = target_annual_income
        years_income_lasted = 0

        for _ in range(RETIREMENT_HORIZON_YEARS):
            if portfolio_value <= 0:
                break

            annual_withdrawal *= 1 + INFLATION_RATE
            portfolio_value = (
                portfolio_value * (1 + _random_portfolio_return(asset_allocation, rng))
                - annual_withdrawal
            )

            if portfolio_value > 0:
                years_income_lasted += 1

        final_values.append(max(0.0, portfolio_value))
        years_lasted.append(years_income_lasted)

        if years_income_lasted >= RETIREMENT_HORIZON_YEARS:
            successful_scenarios += 1

    final_values.sort()
    expected_return = _blended_expected_return(asset_allocation)
    expected_value_at_retirement = current_value
    for _ in range(years_until_retirement):
        expected_value_at_retirement = expected_value_at_retirement * (1 + expected_return)
        expected_value_at_retirement += annual_contribution

    return {
        "success_rate": round((successful_scenarios / num_simulations) * 100, 1),
        "median_final_value": round(final_values[num_simulations // 2], 2),
        "percentile_10": round(final_values[num_simulations // 10], 2),
        "percentile_90": round(final_values[9 * num_simulations // 10], 2),
        "average_years_lasted": round(sum(years_lasted) / len(years_lasted), 1),
        "expected_value_at_retirement": round(expected_value_at_retirement, 2),
    }


def generate_projections(
    current_value: float,
    years_until_retirement: int,
    asset_allocation: Dict[str, float],
    current_age: int,
    annual_contribution: float,
    target_annual_income: float,
) -> list:
    """Year-by-year projections at 5-year milestones (aligned with Monte Carlo assumptions)."""
    expected_return = _blended_expected_return(asset_allocation)
    portfolio_value = current_value
    annual_withdrawal = float(target_annual_income)

    milestone_years = set(range(0, years_until_retirement + RETIREMENT_HORIZON_YEARS + 1, 5))
    milestone_years.add(years_until_retirement)

    projections = []
    for year in range(years_until_retirement + RETIREMENT_HORIZON_YEARS + 1):
        if year > 0:
            if year <= years_until_retirement:
                portfolio_value = portfolio_value * (1 + expected_return) + annual_contribution
            else:
                annual_withdrawal *= 1 + INFLATION_RATE
                portfolio_value = portfolio_value * (1 + expected_return) - annual_withdrawal

        if year not in milestone_years:
            continue

        if portfolio_value <= 0:
            continue

        if year <= years_until_retirement:
            phase = "accumulation"
            income = 0.0
        else:
            phase = "retirement"
            income = annual_withdrawal

        projections.append(
            {
                "year": year,
                "age": current_age + year,
                "portfolio_value": round(portfolio_value, 2),
                "annual_income": round(income, 2),
                "phase": phase,
            }
        )

    return projections


def compute_retirement_metrics(
    portfolio_data: Dict[str, Any],
    user_preferences: Dict[str, Any],
    job_id: Optional[str] = None,
) -> Dict[str, Any]:
    """Deterministic retirement metrics (source of truth for UI and Reporter)."""
    years_until_retirement = int(user_preferences.get("years_until_retirement", 30))
    target_income = float(user_preferences.get("target_retirement_income", 80_000))
    current_age = int(user_preferences.get("current_age", 40))
    annual_contribution = float(user_preferences.get("annual_contribution", 10_000))

    portfolio_value = calculate_portfolio_value(portfolio_data)
    allocation = calculate_asset_allocation(portfolio_data)
    seed = (hash(job_id) & 0xFFFFFFFF) if job_id else None

    monte_carlo = run_monte_carlo_simulation(
        portfolio_value,
        years_until_retirement,
        target_income,
        allocation,
        annual_contribution=annual_contribution,
        num_simulations=500,
        seed=seed,
    )
    projections = generate_projections(
        portfolio_value,
        years_until_retirement,
        allocation,
        current_age,
        annual_contribution,
        target_income,
    )

    return {
        "portfolio_value": round(portfolio_value, 2),
        "allocation": {k: round(v, 4) for k, v in allocation.items()},
        "years_until_retirement": years_until_retirement,
        "target_retirement_income": target_income,
        "current_age": current_age,
        "annual_contribution": annual_contribution,
        "monte_carlo": monte_carlo,
        "projections": projections,
    }


def create_agent(
    job_id: str,
    portfolio_data: Dict[str, Any],
    user_preferences: Dict[str, Any],
    db=None,
):
    """Create the retirement agent with tools and context."""
    model_id = os.getenv("BEDROCK_MODEL_ID", "us.anthropic.claude-3-7-sonnet-20250219-v1:0")
    bedrock_region = os.getenv("BEDROCK_REGION", "us-west-2")
    os.environ["AWS_REGION_NAME"] = bedrock_region

    model = LitellmModel(model=f"bedrock/{model_id}")

    metrics = compute_retirement_metrics(portfolio_data, user_preferences, job_id=job_id)
    portfolio_value = metrics["portfolio_value"]
    allocation = metrics["allocation"]
    monte_carlo = metrics["monte_carlo"]
    projections = metrics["projections"]
    years_until_retirement = metrics["years_until_retirement"]
    target_income = metrics["target_retirement_income"]
    current_age = metrics["current_age"]
    annual_contribution = metrics["annual_contribution"]

    tools = []

    task = f"""
# Portfolio Analysis Context

## Current Situation
- Portfolio Value: ${portfolio_value:,.0f}
- Asset Allocation: {", ".join([f"{k.title()}: {v:.0%}" for k, v in allocation.items() if v > 0])}
- Years to Retirement: {years_until_retirement}
- Target Annual Income: ${target_income:,.0f}
- Current Age: {current_age}
- Assumed Annual Contribution: ${annual_contribution:,.0f}

## Monte Carlo Simulation Results (500 scenarios)
- Success Rate: {monte_carlo["success_rate"]}% (probability of sustaining retirement income for 30 years)
- Expected Portfolio Value at Retirement: ${monte_carlo["expected_value_at_retirement"]:,.0f}
- 10th Percentile Outcome: ${monte_carlo["percentile_10"]:,.0f} (worst case)
- Median Final Value: ${monte_carlo["median_final_value"]:,.0f}
- 90th Percentile Outcome: ${monte_carlo["percentile_90"]:,.0f} (best case)
- Average Years Portfolio Lasts: {monte_carlo["average_years_lasted"]} years

## Key Projections (Milestones)
"""

    for proj in projections[:8]:
        if proj["phase"] == "accumulation":
            task += f"- Age {proj['age']}: ${proj['portfolio_value']:,.0f} (building wealth)\n"
        else:
            task += f"- Age {proj['age']}: ${proj['portfolio_value']:,.0f} (annual income: ${proj['annual_income']:,.0f})\n"

    task += f"""

## Risk Factors to Consider
- Sequence of returns risk (poor returns early in retirement)
- Inflation impact ({INFLATION_RATE:.0%} assumed on withdrawals)
- Healthcare costs in retirement
- Longevity risk (living beyond 30 years)
- Market volatility (equity standard deviation: {EQUITY_RETURN_STD:.0%})

## Safe Withdrawal Rate Analysis
- 4% Rule: ${portfolio_value * 0.04:,.0f} initial annual income
- Target Income: ${target_income:,.0f}
- Gap: ${target_income - (portfolio_value * 0.04):,.0f}

Your task: Analyze this retirement readiness data and provide a comprehensive retirement analysis including:
1. Clear assessment of retirement readiness
2. Specific recommendations to improve success rate
3. Risk mitigation strategies
4. Action items with timeline

Provide your analysis in clear markdown format with specific numbers and actionable recommendations.
The Monte Carlo success rate above is authoritative — your conclusions must match it.
"""

    return model, tools, task, metrics
