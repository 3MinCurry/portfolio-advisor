...
RISK_FUNCTION = os.getenv("RISK_FUNCTION", "alex-risk")
...
async def invoke_risk_internal(job_id: str) -> str:
    """Invoke the Risk Management Lambda for portfolio risk assessment."""
    result = await invoke_lambda_agent("Risk", RISK_FUNCTION, {"job_id": job_id})

    if "error" in result:
        return f"Risk agent failed: {result['error']}"

    return "Risk agent completed successfully. Risk assessment has been saved."
...
@function_tool
async def invoke_risk(wrapper: RunContextWrapper[PlannerContext]) -> str:
    """Invoke the Risk Management agent for concentration and diversification analysis."""
    return await invoke_risk_internal(wrapper.context.job_id)
