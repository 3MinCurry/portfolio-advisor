...
def run_retrieval_evaluation(progress=gr.Progress()):
    ...
    if count == 0:
        return (
            "<div style='padding:20px;color:red;'>No tests loaded. Check evaluation/tests.json.</div>",
            pd.DataFrame(),
        )
...
def main():
    ...
    with gr.Blocks(title="SEC 10-K RAG Evaluation Dashboard") as app:
        gr.Markdown("# 📊 SEC 10-K RAG Evaluation Dashboard")
        gr.Markdown(
            "Evaluate retrieval and answer quality for the SEC 10-K filings RAG system "
            "on the S&P 500 eval sets in `evaluation/tests.json`."
        )

        # RETRIEVAL SECTION
        gr.Markdown("## 🔍 Retrieval Evaluation")
        gr.Markdown(
            "_Metrics: MRR (Mean Reciprocal Rank), nDCG (Normalized Discounted Cumulative Gain), "
            "and Keyword Coverage. All computed against the expected keywords in each test._"
        )
        ...
        gr.Markdown(
            "_LLM-as-a-judge scoring (1-5) on accuracy, completeness, and relevance. "
            "Judge model: gpt-4.1._"
        )
    ...
    # Gradio 6 accepts `theme` here; Gradio 5 ignores it harmlessly here
    # since it's already applied via Blocks() in older versions. Try here first,
