...
if SQS_QUEUE_URL:
    logger.info("SQS analysis queue configured (jobs will reach alex-planner)")
else:
    logger.warning(
...
class PopulateTestDataRequest(BaseModel):
    scenario: str = Field(
        ...
        description="Test portfolio scenario id (see GET /api/test-data-scenarios)",
    )
...
@app.get("/api/test-data-scenarios")
async def get_test_data_scenarios():
    """List available demo portfolio scenarios."""
...
async def populate_test_data(
    ...
    """Populate test portfolio for the current user (choose a scenario)."""
    ...
    try:
        ...
        existing = db.accounts.find_by_user(clerk_user_id)
        if existing:
            raise HTTPException(
                status_code=400,
                detail="User already has accounts. Reset accounts first, then load test data.",
            )

        return apply_scenario(db, clerk_user_id, request.scenario)
