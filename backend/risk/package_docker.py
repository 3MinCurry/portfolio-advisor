#!/usr/bin/env python3
"""
Package the risk Lambda function using Docker for AWS compatibility.
"""
...
def package_lambda():
    ...
    risk_dir = Path(__file__).parent.absolute()
    backend_dir = risk_dir.parent
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        shutil.copy(risk_dir / "lambda_handler.py", package_dir)
        shutil.copy(risk_dir / "agent.py", package_dir)
        shutil.copy(risk_dir / "templates.py", package_dir)
        shutil.copy(risk_dir / "observability.py", package_dir)
        
        # Create the zip file
        zip_path = risk_dir / "risk_lambda.zip"
...
def main():
    parser = argparse.ArgumentParser(description='Package risk Lambda for deployment')
