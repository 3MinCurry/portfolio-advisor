#!/usr/bin/env python3
"""
Deploy all agent Lambda functions to AWS using Terraform.
...
def taint_and_deploy_via_terraform() -> bool:
    ...
    lambda_functions = ['planner', 'tagger', 'reporter', 'charter', 'retirement', 'risk']
    
    print(" Tainting Lambda functions to force recreation...")
    print("-" * 50)
...
def main():
    ...
    services = [
        ...
        ('risk', backend_dir / 'risk' / 'risk_lambda.zip'),
    ]
    ...
    if taint_and_deploy_via_terraform():
        ...
        print()
        print("  Lambda functions were recreated with the latest packages.")
        sys.exit(0)
    else:
        print()
