...
resource "aws_s3_object" "lambda_packages" {
  for_each = toset(["planner", "tagger", "reporter", "charter", "retirement", "risk"])
...
# Risk Management Lambda
resource "aws_lambda_function" "risk" {
  function_name = "alex-risk"
  role          = aws_iam_role.lambda_agents_role.arn

  s3_bucket        = aws_s3_bucket.lambda_packages.id
  s3_key           = aws_s3_object.lambda_packages["risk"].key
  source_code_hash = fileexists("${path.module}/../../backend/risk/risk_lambda.zip") ? filebase64sha256("${path.module}/../../backend/risk/risk_lambda.zip") : null
  ...
  depends_on = [aws_s3_object.lambda_packages["risk"]]
}
